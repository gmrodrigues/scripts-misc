...
while (<>) {
    ...
    my $filename = "$CSV_PATH/$table_name.csv";
    my $copy_to_cmd = "COPY $KEYSPACE.$table_name TO   $filename;";
    my $copy_from_cmd = "COPY $KEYSPACE.$table_name FROM $filename;";
    print "\n#table table_name from $FROM_SERVER to $TO_SERVER\n";
    print "\n$CQLCMD $FROM_SERVER -e \"$copy_from_cmd\"\n";
    print "\n$CQLCMD $TO_SERVER -e \"$copy_to_cmd\"\n";
}
